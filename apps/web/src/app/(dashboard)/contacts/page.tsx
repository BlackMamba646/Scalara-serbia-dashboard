...
import { SEED_CONTACTS } from "@/lib/seed-data";

export default function ContactsPage() {
  const [search, setSearch] = useState("");

  const filtered = SEED_CONTACTS.filter((c) => {
  ...
  const decisionMakers = SEED_CONTACTS.filter((c) => c.isDecisionMaker).length;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold tabular-nums">{SEED_CONTACTS.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold tabular-nums text-primary">
              {SEED_CONTACTS.filter((c) => c.email).length}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <TableBody>
              {filtered.map((contact) => (
                <TableRow key={contact.id}>
                  ...
                  <TableCell>
                    <span className="text-xs font-medium tabular-nums">{contact.confidence}%</span>
                  </TableCell>
